...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const userSignup = await API.post("user/signup", {
        username: formData.username,
        emailId: formData.email,
        password: formData.password,
      });
      if (userSignup.data.status == 'Success') {
        ...
        <motion.div variants={itemVariants} className="w-full max-w-sm bg-[#0F0F0F] border border-white/5 rounded-2xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full mt-4 py-2.5 px-4 rounded-lg bg-white hover:bg-white/90 disabled:opacity-70 disabled:cursor-not-allowed text-sm text-[#0A0A0A] font-semibold transition-all duration-200 flex items-center ${
                isLoading ? "justify-center gap-2" : "justify-between group"
              }`}
            >
              {isLoading ? (
                <>
